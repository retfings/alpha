...
由于 MoonBit C 后端的参数传递机制，CLI 命令需要通过环境变量方式传递参数：
...
- `--stock`: 股票代码，格式为 `market.code`（如 `sh.600000`）
- `--metric`: 分析指标（如 `max_drawdown`）
- `--strategy`: 策略名称（如 `ma_cross`, `momentum`）
- `--start`: 开始日期（格式：`YYYY-MM-DD`）
- `--end`: 结束日期（格式：`YYYY-MM-DD`）
- `--format`: 报告格式（`text`, `html`, `json`, `pdf`）
- `--port`: 服务器端口（默认：8080）
